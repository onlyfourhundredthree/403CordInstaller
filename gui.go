...
var (
	...
	CordTeal    = color.RGBA{R: 0x00, G: 0xBD, B: 0x9D, A: 0xFF}
	CordBg      = color.RGBA{R: 0x0F, G: 0x0F, B: 0x0F, A: 0xFF}
	CordSurface = color.RGBA{R: 0x1A, G: 0x1A, B: 0x1A, A: 0xFF}
)
...
func main() {
	...
	win = g.NewMasterWindow("403Cord Installer", 1200, 800, 0)
...
func InstallLatestBuilds() (err error) {
	...
	if err != nil {
		ShowModal("Uh Oh!", "Failed to install the latest 403Cord builds from GitHub:\n"+err.Error())
	}
	return
}
...
func renderInstaller() g.Widget {
	...
	layout := g.Layout{
		...
		g.Style().SetFontSize(20).To(
			renderErrorCard(
				DiscordYellow,
				"**Github** and **403cord.dev** are the only official places to get 403Cord. Any other site claiming to be us is malicious.\n"+
				...
				g.Style().
					...
					To(
						g.Button("Reinstall / Repair").
							...
							Size((w-40)/4, 50),
						Tooltip("Reinstall & Update 403Cord"),
					),
				g.Style().
		...
		InfoModal("#patched", "Successfully Patched", "If Discord is still open, fully close it first.\n"+
			"Then, start it and verify 403Cord installed successfully by looking for its category in Discord Settings"),
		...
		InfoModal("#scuffed-install", "Hold On!", "You have a broken Discord Install.\n"+
			...
			"You need to fix this before patching, otherwise 403Cord will likely not work.\n\n"+
		...
		RawInfoModal("#openasar-confirm", "OpenAsar", "OpenAsar is an open-source alternative of Discord desktop's app.asar.\n"+
			"403Cord is in no way affiliated with OpenAsar.\n"+
