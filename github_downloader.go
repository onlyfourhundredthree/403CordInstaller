...
func GetGithubRelease(url, fallbackUrl string) (*GithubRelease, error) {
	...
	if res.StatusCode >= 300 {
		...
		// If that is the case, try our fallback at https://403cord.dev/releases/project
...
func InitGithubDownloader() {
	...
	IsDevInstall = os.Getenv("403CORD_DEV_INSTALL") == "1"
	...
	CordFile := CordDirectory

	stat, err := os.Stat(CordFile)
	if err != nil {
		return
	}
	...
	// dev
	if stat.IsDir() {
		CordFile = path.Join(CordFile, "main.js")
	}

	// Check hash of installed version if exists
	b, err := os.ReadFile(CordFile)
	if err != nil {
		return
	}

	Log.Debug("Found existing 403Cord Install. Checking for hash...")

	re := regexp.MustCompile(`// 403Cord (\w+)`)
...
func installLatestBuilds() (retErr error) {
	...
	out, err := os.OpenFile(CordDirectory, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
		Log.Error("Failed to create", CordDirectory+":", err)
		retErr = err
		return
	}
	read, err := io.Copy(out, res.Body)
	if err != nil {
		Log.Error("Failed to download to", CordDirectory+":", err)
		retErr = err
		return
	}
